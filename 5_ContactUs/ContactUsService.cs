...
namespace amana_mono._5_ContactUs
{
    public class ContactUsService : IContactUsService
    {
        ...
        public ContactUsDto Add(CreateNewsInput input)
        {
            var news = _mapper.Map<ContactUs>(input);
            var createdNews = _crudService.Add<ContactUs, Guid>(news);
            _crudService.SaveChanges();
            return _mapper.Map<ContactUsDto>(createdNews);
        }
        public List<ContactUsDto> AddMany(List<CreateContactUsInput> inputs)
        {
            var newsList = _mapper.Map<List<ContactUs>>(inputs);
            var createdNewsList = _crudService.AddAndGetRange<ContactUs, Guid>(newsList);
            _crudService.SaveChanges();
            return _mapper.Map<List<ContactUsDto>>(createdNewsList);
        }

        public ContactUsDto Update(UpdateContactUsInput input)
        {
            var news = _mapper.Map<ContactUs>(input);
            var updatedNews = _crudService.Update<ContactUs, Guid>(news);
            _crudService.SaveChanges();
            return _mapper.Map<ContactUsDto>(updatedNews);
        }
        public List<ContactUsDto> UpdateMany(List<UpdateContactUsInput> inputs)
        {
            var newsList = _mapper.Map<List<ContactUs>>(inputs);
            var updatedNewsList = _crudService.UpdateAndGetRange<ContactUs, Guid>(newsList);
            _crudService.SaveChanges();
            return _mapper.Map<List<ContactUsDto>>(updatedNewsList);
        }

        public bool Delete(Guid id)
        {
            var contacts = _crudService.Find<ContactUs, Guid>(id);
            if (contacts is not null)
            {
                _crudService.SoftDelete<ContactUs, Guid>(news);
                _crudService.SaveChanges();
                return true;
            }
        ...
        public bool Activate(Guid id)
        {
            ...
            if (contacts is not null)
            {
                _crudService.Activate<ContactUs, Guid>(news);
                _crudService.SaveChanges();
                return true;
            }
            _errorMessage = $"News record Not Found!!!";
